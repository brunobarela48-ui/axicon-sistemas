...
// Remetente padrão (sobrescrevível por chamada). O domínio precisa estar
// autenticado no Brevo — no nosso caso, axiconsolucoes.com.
function defaultSender() {
  return {
    name:  process.env.BREVO_SENDER_NAME  || 'Áxicon Soluções Financeiras',
    email: process.env.BREVO_SENDER_EMAIL || 'contato@axiconsolucoes.com',
  }
}

// Aceita 'a@b.com' | {email,name} | array de qualquer um dos dois.
 ...
 */
export async function sendEmail({ to, subject, html, text, replyTo, sender, params, tags, cc, bcc }) {
 ...
 */
export async function sendBulk(recipients, { subject, html, text, replyTo, sender, tags, concurrency = 5 } = {}) {
